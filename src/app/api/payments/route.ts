...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!paymentData.jobId && !paymentData.gigId) {
      return NextResponse.json({ error: "Either jobId or gigId must be provided" }, { status: 400 });
    }

    if (paymentData.jobId && paymentData.gigId) {
      return NextResponse.json({ error: "Cannot specify both jobId and gigId" }, { status: 400 });
    }
    ...
    const payment = await prisma.payment.create({
      data: {
        ...
        escrowAddress: `escrow_${Date.now()}_${Math.random().toString(36).substring(2, 15)}`, // Mock address
      },
      include: {
        fromUser: {
          select: {
